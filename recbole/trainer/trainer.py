...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        if self.AutoML:
            self.optimizer, self.darts_optimizer = self._build_optimizer(self.model.named_parameters())
    ...
    def _build_optimizer(self, params):
        ...
        if self.AutoML:
            optimizer_model = optim.Adam(params=[param for name, param in self.model.named_parameters() if name != "weights.deep_weights"], lr=self.learning_rate, weight_decay=self.weight_decay)
            optimizer_darts = optim.Adam(params=[param for name, param in self.model.named_parameters() if name == "weights.deep_weights"], lr=0.001, weight_decay=self.weight_decay)
            return optimizer_model, optimizer_darts
        else:
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            if self.AutoML:
                ...
                # scaler.step(self.darts_optimizer)
                scaler.update()
                if self.gpu_available and show_progress:
                    iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
                # print("weight:",self.model.weights.deep_weights)
            else:
                interaction = interaction.to(self.device)
                self.optimizer.zero_grad()
                with torch.cuda.amp.autocast():
    ...
    def _valid_epoch(self, valid_data, show_progress=False):
        ...
        if self.AutoML:
            ...
            total_loss = None
            for batch_idx, interaction in enumerate(iter_data):
                interaction = interaction[0].to(self.device)
                self.darts_optimizer.zero_grad()
                with torch.cuda.amp.autocast():
                    losses = loss_func(interaction)
                if isinstance(losses, tuple):
                    loss = sum(losses)
